...
namespace aim.FullSerializer
{
    ...
    public abstract class fsBaseConverter
    {
        ...
        public abstract fsResult TrySerialize(object instance, out fsData serialized, Type storageType, object other);
        ...
        /// </returns>
        public abstract fsResult TryDeserialize(fsData data, ref object instance, Type storageType, object other);
        ...
        protected fsResult SerializeMember<T>(Dictionary<string, fsData> data, Type overrideConverterType, string name, T value, object other)
        {
            fsData memberData;
            var result = Serializer.TrySerialize(typeof(T), overrideConverterType, value, out memberData, other);
            if (result.Succeeded) data[name] = memberData;
            return result;
        }

        protected fsResult DeserializeMember<T>(Dictionary<string, fsData> data, Type overrideConverterType, string name, out T value, object other)
        {
            fsData memberData;
            ...
            object storage = null;
            var result = Serializer.TryDeserialize(memberData, typeof(T), overrideConverterType, ref storage, other);
            value = (T)storage;
            return result;
        }
    }
