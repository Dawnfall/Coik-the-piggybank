...
namespace aim.FullSerializer.Internal
{
    ...
    public class fsDictionaryConverter : fsConverter
    {
        ...
        public override fsResult TryDeserialize(fsData data, ref object instance_, Type storageType, object other)
        {
            ...
            if (data.IsList)
            {
                ...
                for (int i = 0; i < list.Count; ++i)
                {
                    ...
                    if ((result += Serializer.TryDeserialize(keyData, keyStorageType, ref keyInstance, other)).Failed) return result;
                    if ((result += Serializer.TryDeserialize(valueData, valueStorageType, ref valueInstance, other)).Failed) return result;
        ...
        public override fsResult TrySerialize(object instance_, out fsData serialized, Type storageType, object other)
        {
            ...
            {
                fsData keyData, valueData;
                if ((result += Serializer.TrySerialize(keyStorageType, enumerator.Key, out keyData, other)).Failed) return result;
                if ((result += Serializer.TrySerialize(valueStorageType, enumerator.Value, out valueData, other)).Failed) return result;
